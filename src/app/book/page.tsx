      ...
      <Section className="space-y-4 text-center">
        <Reveal className="space-y-4">
          <MagicAura intensity="medium" colorVariant="lavender" trigger="view" className="block">
            <h1 className="text-4xl sm:text-5xl magic-title">{intro.title}</h1>
          </MagicAura>
          <p className="mx-auto max-w-2xl text-sm text-night-800 magic-dropcap">{intro.subtitle}</p>
        </Reveal>
      </Section>
        ...
        <Section key={section.title} className="space-y-6">
          <Reveal className="space-y-3">
            <MagicAura intensity="soft" colorVariant="rose" trigger="view" className="block">
              <h2 className="text-3xl sm:text-4xl magic-title">{section.title}</h2>
            </MagicAura>
          ...
          <Stagger className="grid gap-6 md:grid-cols-2" stagger={0.1}>
            {section.cards.map((card) => (
              <StaggerItem key={card.title} variant="scaleIn" hoverLift className="h-full">
                <MagicAura intensity="soft" colorVariant="rose" trigger="hover" className="block h-full">
